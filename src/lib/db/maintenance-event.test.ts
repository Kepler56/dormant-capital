...
describe("maintenance_event store", () => {
  it("inserts and reads back events for a patent number", async () => {
  ...
  it("prevents duplicate events on re-insert (idempotent)", async () => {
    ...
    await insertMaintenanceEvents(events);
    await insertMaintenanceEvents(events);
    const rows = await getMaintenanceEvents("5555555");
    expect(rows).toHaveLength(2); // Still 2, not 4
  });

  it("returns empty list for unknown patent number", async () => {
    const rows = await getMaintenanceEvents("9999999");
    expect(rows).toEqual([]);
  });
});
