...
import { run } from "../../src/lib/db/connection";
...
export async function materializePatent(b: PatentBundle, urls: { fee: string; gPatent: string; assignee: string; cpc: string }, retrievedAt: string): Promise<void> {
  ...
  await insertMaintenanceEvents(eventRows);
  ...
  await run(
    ...
    [externalNumber, Number.isFinite(year as number) ? year : null, b.title, b.assignee]
  );

  // 3. asset + immutable facts (idempotent: skip if facts already exist)
  const assetId = await upsertAsset(externalNumber);
  if ((await getFacts(assetId)).length === 0) {
    const legalEvents = b.events.map((e) => ({ date: e.eventDate ?? "", code: e.eventCode, description: describeCode(e.eventCode) }));
    const fact = async (key: string, value: unknown, source: string, url: string) => {
      if (value != null && (!Array.isArray(value) || value.length))
        await insertFact(assetId, { key, value, source, sourceUrl: url, retrievedAt });
    };
    await fact("title", b.title, BIBLIO_SOURCE, urls.gPatent);
    await fact("assignee", b.assignee, BIBLIO_SOURCE, urls.assignee);
    await fact("grant_date", b.grantDate, BIBLIO_SOURCE, urls.gPatent);
    await fact("filing_date", b.filingDate, FEE_SOURCE, urls.fee);
    await fact("entity_status", b.entityStatus, FEE_SOURCE, urls.fee);
    await fact("cpc_classes", b.cpc, BIBLIO_SOURCE, urls.cpc);
    await fact("legal_events", legalEvents, FEE_SOURCE, urls.fee);
    // boolean fact: write unconditionally (insertFact's guard would drop `false`)
    await insertFact(assetId, { key: "maintenance_lapsed", value: lapsed, source: FEE_SOURCE, sourceUrl: urls.fee, retrievedAt });

    await appendEvent("loaded", assetId, { patentNumber: externalNumber, events: b.events.length, lapsed });
  }
}
